 ...
 *
 * Registers:
 *   wp parcel version  — theme name and version, read from PARCEL_SLUG + PARCEL_VERSION.
 *   wp parcel info     — name, version, active template, .pot presence.
 *   wp parcel flush    — wp_cache_flush().
 *
 * @package parcel
 */
...
class Parcel_CLI_Command {
	...
	public function version(): void {
		$theme = wp_get_theme();
		\WP_CLI::log( sprintf( '%s %s', (string) $theme->get( 'Name' ), PARCEL_VERSION ) );
	}
	...
	public function info(): void {
		...
		$languages_dir = PARCEL_DIR . '/languages';
		$pot_path      = $languages_dir . '/' . PARCEL_SLUG . '.pot';
		$has_pot       = is_dir( $languages_dir ) && file_exists( $pot_path );

		\WP_CLI::log( 'Name:            ' . (string) $theme->get( 'Name' ) );
		\WP_CLI::log( 'Slug:            ' . PARCEL_SLUG );
		\WP_CLI::log( 'Version:         ' . PARCEL_VERSION );
		\WP_CLI::log( 'Active template: ' . $template );
		\WP_CLI::log( 'languages/.pot:  ' . ( $has_pot ? 'present' : 'missing — run: wp i18n make-pot . languages/' . PARCEL_SLUG . '.pot' ) );
	}
...
\WP_CLI::add_command( 'parcel', 'Parcel_CLI_Command' );
