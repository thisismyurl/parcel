 ...
 *
 * Portable: every handle derives from the PARCEL_SLUG constant, so a re-skin needs no
 ...
 *
 * Cache-buster is each file's modification time, not the PARCEL_VERSION constant, so a
 * CSS edit busts caches immediately during iteration without a version bump.
 */
function parcel_enqueue_assets(): void {
...
add_action( 'wp_enqueue_scripts', 'parcel_enqueue_assets' );
 ...
 * an off-origin fetch.
 */
function parcel_preload_fonts(): void {
	/**
	 ...
	 * @since 1.0.0
	 */
	$fonts = (array) apply_filters( PARCEL_SLUG . '/preload_fonts', array() );
	...
	foreach ( $fonts as $font ) {
		...
		printf(
			...
			esc_url( PARCEL_URI . '/' . ltrim( $font, '/' ) )
		);
	}
}
add_action( 'wp_head', 'parcel_preload_fonts', 1 );
