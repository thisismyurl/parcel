 ...
 *
 * This is the ONE place a theme's identity lives. Change the `parcel_` function
 * prefix and the PARCEL_* constants and the entire theme re-prefixes, because
 ...
 * them honest.
 *
 * Why a `parcel_` function prefix and not a namespace (reversed 2026-07-20):
 * this line originally read `namespace Parcel;`, on the reasoning that callbacks
 ...
 * rewrites `parcel_` → `<slug>_`, `PARCEL_` → `<SLUG>_`, and `Parcel_` →
 ...
 * stays a string LITERAL ('parcel'), never the PARCEL_SLUG constant. `wp i18n
 ...
 */
define( 'PARCEL_DIR', get_template_directory() );

/**
 * Public URL to the theme root (no trailing slash).
 */
define( 'PARCEL_URI', get_template_directory_uri() );
