 ...
 */
function parcel_setup(): void {

	// i18n. The domain is the literal 'parcel' (a constant would break
	// make-pot — see bootstrap.php); the path uses PARCEL_DIR so it travels with a
	// re-skin. The CLI rewrites the literal when it generates a theme.
	load_theme_textdomain( 'parcel', PARCEL_DIR . '/languages' );
	 ...
	 */
	$GLOBALS['content_width'] = (int) apply_filters( PARCEL_SLUG . '/content_width', 720 );
	...
	register_nav_menus(
		(array) apply_filters(
			PARCEL_SLUG . '/register_nav_menus',
			array(
				'primary' => esc_html__( 'Primary Navigation', 'parcel' ),
				'footer'  => esc_html__( 'Footer Navigation', 'parcel' ),
			)
		)
	);
	 ...
	 * @since 1.0.0
	 */
	do_action( PARCEL_SLUG . '/setup' );
}
add_action( 'after_setup_theme', 'parcel_setup' );
 ...
 * plugin is active.
 */
function parcel_woocommerce_support(): void {
...
add_action( 'after_setup_theme', 'parcel_woocommerce_support' );
 ...
 */
function parcel_editor_styles(): void {
	add_editor_style( array( 'assets/css/editor-style.css' ) );
}
add_action( 'after_setup_theme', 'parcel_editor_styles' );
 ...
 */
function parcel_comment_form_field_attributes( array $fields ): array {
	$attributes = array(
...
add_filter( 'comment_form_default_fields', 'parcel_comment_form_field_attributes' );
